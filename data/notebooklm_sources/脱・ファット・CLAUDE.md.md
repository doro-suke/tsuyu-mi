# 脱・ファット・CLAUDE.md
- **Source URL**: https://zenn.dev/smartshopping/articles/refactor-fat-claude-md
- **Priority**: high
- **AI Summary**:
  - CLAUDE.mdの肥大化を防ぎ、不要な文脈によるAIの回答精度低下（チェーホフの銃の誤謬）を回避する手法の解説。
  - ディレクトリ階層ごとの自動読み込みルールを活用し、プロンプトをプロジェクト全体や各機能単位に分散配置する。
  - ワークフローをスラッシュコマンドやサブエージェントへ移行し、メインセッションのトークン消費を効率化する。
- **Read Now Reason**: Claude Codeを利用した開発において、回答精度とトークン効率を即座に改善できる具体的なリファクタリング手法が示されているため。
- **Suggested Tags**: #ClaudeCode, #PromptEngineering
...
夏なのでCLAUDE.mdのダイエット方法についてまとめました。
 ...
 CLAUDE.mdとは何か
CLAUDE.mdとは、Claude Codeのメモリ機能です。大きく分けてユーザーメモリとプロジェクトメモリがあります。
...
プロジェクトメモリ(祖先)
プロジェクトからルートディレクトリまでの CLAUDE.md(/ 自身は除く)
プロジェクト起動時
...
https://amzn.asia/d/8cQjTrU ↩︎


https://docs.anthropic.com/en/docs/claude-code/memory#determine-memory-type ↩︎


https://arxiv.org/pdf/2507.13334 ↩︎


https://docs.anthropic.com/en/docs/claude-code/memory#how-claude-looks-up-memories ↩︎


https://docs.anthropic.com/en/docs/claude-code/slash-commands#custom-slash-commands ↩︎


https://docs.anthropic.com/en/docs/claude-code/sub-agents ↩︎


https://www.anthropic.com/engineering/claude-code-best-practices ↩︎
