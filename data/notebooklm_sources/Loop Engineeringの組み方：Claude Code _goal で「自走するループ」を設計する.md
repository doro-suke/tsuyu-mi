# Loop Engineeringの組み方：Claude Code /goal で「自走するループ」を設計する
- **Source URL**: https://zenn.dev/ino_h/articles/2026-06-16-loop-engineering-goal
- **Score**: 88
- **AI Summary**:
  - 人間を介さず自走するAIエージェントのループを設計する思想と構成要素の解説
  - Claude Codeの`/goal`コマンドを活用した、自走ループの実行方法と具体的設定例
  - 評価器の制約を理解した、測定可能で安全な終了条件（ゴール）の具体的な設計手法
- **Read Now Reason**: AI駆動開発と自動化パイプライン構築において、人間が1ターンごとにプロンプトを入力する手間を排除し、自走型でタスクを完遂させる「Loop Engineering」の思想と、Claude Codeの`/goal`による具体的な実装・制御方法が明記されているため。特に評価器の制約（コンテキスト内の出力のみで判定する点）や、暴走を防ぐターン制限（or stop after 20 turns）などの実践的なパラメータ設定は即時導入可能です。
- **Suggested Tags**: #Loop-Engineering, #Claude-Code
