# 【Claude Code】別々のセッション同士を会話させるSkillを作った ─ 「ありがとう→いえいえ」無限ループを止めるまで - Qiita
- **Source URL**: https://qiita.com/kawabe0201/items/2bc126be3502138698fb
- **Score**: 85
- **AI Summary**:
  - 共有フォルダとプロトコルを用いた、Claude Code複数セッション間の非同期通信の実装方法
  - 無限ループを防ぐメッセージタイプ定義と、最大往復制限によるトークン消費とコストの抑制対策
  - バックグラウンド監視によるトークン消費ゼロの待機手法や、時刻ベースの衝突回避設計の実践
- **Read Now Reason**: AI駆動開発において、エージェント同士の連携時に発生する『無限ループ（お礼合戦）』によるトークン浪費を防ぐメッセージプロトコルや、ファイル衝突を避けるタイムスタンプ設計、待機中のトークン消費をゼロにするバックグラウンド処理の具体的なアプローチが明記されているため。
- **Suggested Tags**: #Claude Code, #AIエージェント協調, #プロトコル設計, #トークン最適化
...
返信していいのは、自分宛ての request / task だけ
response / info / status には返信しない。お礼・了解・挨拶だけのメッセージを送らない
