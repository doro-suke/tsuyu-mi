...
- **Source URL**: https://zenn.dev/linkedge/articles/27c38cdd9bedc6
- **Priority**: high
- **AI Summary**:
  - Claude Codeを「育成」し、的外れな提案や重複説明の手間を解消するための具体的設定手法を解説。
  - CLAUDE.mdによるプロジェクト前提知識の共有、skillsによるタスク手順の定義で開発効率を劇的に改善。
  - agentsディレクトリを用いた専門特化型サブエージェントの構築により、メインコンテキストの圧迫を回避。
- **Read Now Reason**: Claude Codeは現在トレンドのツールであり、導入済みまたは検討中のユーザーが設定ファイル一つで即座に開発効率を向上させられる実践的な内容であるため。
- **Suggested Tags**: #Claude Code, #AI開発ツール, #開発効率化, #プロンプトエンジニアリング
...
どちらも自動的に読み込まれる点では同じ
プロジェクト全体に効かせたいことは CLAUDE.md にまとめる
特定のファイルやディレクトリに限定して効かせたいルールは rules/ に書く、という使い分けが分かりやすい



 commands/ と skills/

新しく作るなら skills/ を優先するのが基本

/コマンド名 で素早く呼び出したいケースでは commands/ も選択肢になる
