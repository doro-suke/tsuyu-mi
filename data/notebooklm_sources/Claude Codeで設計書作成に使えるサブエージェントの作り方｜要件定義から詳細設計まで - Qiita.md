...
- **Source URL**: https://qiita.com/kamome_susume/items/467446c5a6cbd10a5ffa
- **Score**: 85
- **AI Summary**:
  - Claude Codeで設計書を自動生成する際、フェーズ別に5つのサブエージェントに分ける手法を提案。
  - 要件定義から詳細設計、DB設計、Mermaid図表生成までの具体的な設定ファイルのコードを明記。
  - 表記ブレや矛盾を防ぐため、CLAUDE.mdを用いた用語集の共有やコード読込権限の付与を推奨。
- **Read Now Reason**: Claude Codeを用いたAI駆動開発において、即座にコピー＆ペーストして使える5つのサブエージェント設定コードが記載されており、設計自動化の導入とCLAUDE.mdを用いた運用改善に直結するため。
- **Suggested Tags**: #Claude-Code, #AI駆動開発, #設計自動化, #LLMエージェント
...
フェーズ間で内容が矛盾する
各サブエージェントが前段の設計書を見ずに作っている
前フェーズのファイルパスをプロンプトで明示的に渡す


用語の表記がフェーズごとに違う
用語集が共有されていない
プロジェクトのCLAUDE.mdに用語集を書き、全サブエージェントに参照させる


詳細設計が実装と食い違う
コードを読まずに想像で書いている
detailed-designerにはGrep/Globなど読み取り権限を必ず付与する


図が複雑になりすぎる
1つの図に全処理を入れようとしている
「機能単位で図を分けて」と依頼時に指示する
