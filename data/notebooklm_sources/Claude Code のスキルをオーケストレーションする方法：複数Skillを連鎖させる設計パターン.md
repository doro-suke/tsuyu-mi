# Claude Code のスキルをオーケストレーションする方法：複数Skillを連鎖させる設計パターン
- **Source URL**: https://zenn.dev/ino_h/articles/2026-05-15-claude-code-skills-orchestration
- **Score**: 85
- **AI Summary**:
  - 複数Skillsを連鎖させる4つの設計パターン（直列・並列・分岐・反復）と具体的な実装例を提示。
  - 標準仕様とClaude Code固有の拡張を区別し、他クライアントへの移植性における注意点を解説。
  - 状態管理を構造化JSONに一元化することで、エージェント間のパース揺らぎやデータ整合性問題を防止。
- **Read Now Reason**: Claude Codeを用いたAI駆動開発において、自律エージェントや自動化パイプラインを実装する際の具体的なSkills設計パターンと、無限ループやパースエラーを防ぐ状態管理のベストプラクティスが即座に適用できるため。
- **Suggested Tags**: #Claude-Code, #Agent-Skills, #AI駆動開発, #オーケストレーション, #自動化パイプライン
...
Anthropic は Skills を 「composable resources」 と位置づけています[1]。つまり Skills は最初から 複数を組み合わせて使う ことが前提の設計です。本記事では「複数Skillをどう連鎖させ、どこに状態を置き、どこをAIに判断させるか」というオーケストレーション設計に絞って整理します。
...
オーケストレーションの設計に入る前に、Skillの守備範囲を確定させておきましょう。Anthropic公式blogは SkillsとMCPサーバーの使い分けを次のように示しています[2]。
...
Agent Skills 仕様はAnthropicが発端でオープンソース化された規格で、Claude Code以外にも Cursor、Codex、Gemini CLI、VS Code、Copilotなど30以上のクライアントが採用しています。標準フロントマターは以下の通りです。
...
name
Yes
1-64文字、a-z 0-9 -、先頭・末尾ハイフン不可、連続ハイフン不可、親ディレクトリ名と一致
...
仕様書には Skill間連携・依存関係・状態共有に関する明示的な規定は存在しません。連携は仕様の範囲外で、各クライアントが独自拡張で実現する設計になっています。
 ...
 Claude Code固有の拡張
Claude Code Docs で追加されているフロントマターフィールドは以下の通りです。
...
オーケストレーションを支えるのは主に context: fork、hooks、allowed-tools、paths、それから本文中の動的コンテキスト注入（後述）です。これらはClaude Code固有なので、他のSkillsクライアントへの移植性はない という点には注意しておきましょう。
 ...
 Skill連携の4パターン
複数Skillを連鎖させる構造は、おおむね4つに分類できます[3]。
...
1つのSkillがジョブをN個に分解し、サブエージェントで並列処理してマージする構造です。context: fork を使うと各Skill実行が独立コンテキストで走るので、メインの会話文脈を汚しません[4]。
---
...
成功条件を満たすまでSkillを繰り返します。最大反復回数か明確な成功条件を必ず定義する[3:1]——これを忘れると暴走します。
---
...
パース信頼性：プレーンテキストは曖昧で、各Skillが「どこから読むか」で揺れます

真実の源の一意性：複数ファイルに状態を分けると、Skillが古いデータを読むリスクが出ます
...
オーケストレーションの実装で最も効果が大きいのが 「どこに何を書くか」の責務分離 です。playparkの実装例[5]を参考に整理すると次のようになります。
...
決定論的処理
シェルスクリプト
パス計算、日付計算、状態ファイル読み書き、git操作


第2層
条件分岐ロジック
SKILL.md
「stage が X ならこのSkillを呼ぶ」
...
!`cmd` 構文は Claudeに渡る前に シェルでコマンドを実行し、その出力をプロンプトに差し込みます[4:1]。Claudeが実行するのではなく、事前展開という点がポイントです。これを使うと第1層の結果を第3層のAIに自然に渡せます。
...
Claude Code はSkillのフロントマター内で hooks を定義できます[6]。これは そのSkillが有効な間だけ フックが効く、というスコープが付くのが特徴です。
---
...
状態同期：Stop（サブエージェントなら自動で SubagentStop に変換されます）で状態ファイルを永続化します
 ...
 Agent Teams との混同に注意
Claude Codeには「Skillsのオーケストレーション」とは別に Agent Teams という機能があります。これは複数のClaude Codeインスタンスを協調させる仕組みで、現状（2026年5月時点）は experimental で GA未達です。利用には次の条件が必要です[7]。
...
「how → Skill / access → MCP」 の使い分けを徹底する

オープン標準フィールドとClaude Code拡張を区別する（移植性に関わります）

4つの連携パターン（Sequential / Parallel / Conditional / Iterative）から適切なものを選ぶ

状態はJSONファイル1つ に集約する

3層責務分離（決定論的スクリプト / 条件分岐SKILL.md / AI判断）でレイヤーを切る

context: fork + hooks + 動的コンテキスト注入 を組み合わせてオーケストレーションを実装する

Agent Teams とは別レイヤー であることを忘れない
...
Extend Claude with skills - Claude Code Docs ↩︎ ↩︎


【Claude Code】Skillオーケストレーション設計 - 複数Skillを連携させる実践パターン - playpark ↩︎


Hooks reference - Claude Code Docs ↩︎
