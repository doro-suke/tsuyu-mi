# 【Claude Code】マネできる！個人開発するときに最初に用意したドキュメント24種と機能要件書を全公開 #生成AI - Qiita
- **Source URL**: https://qiita.com/tomada/items/e27292b65f723c4633d9
- **Priority**: high
- **AI Summary**:
  - Claude Codeでの開発効率を最大化するため、AIに読み込ませるべき設計書24種の構成を定義。
  - 技術設計、デザインシステム、特定ライブラリ対策など、AIの迷いを防ぐための具体的な構造を網羅。
  - プロンプトを活用し、AI自身にこれらのドキュメント群を自動生成・管理させる再現性の高い手法を紹介。
- **Read Now Reason**: AIエージェントを用いた開発における「指示のブレ」を劇的に減らす具体的かつ即効性のあるドキュメント構成案と、それを自動生成するプロンプトが公開されているため、現在AI開発を試行しているなら必読です。
- **Suggested Tags**: #Claude Code, #生成AI開発, #個人開発, #設計ドキュメント, #AIプロンプト
...
【追記】
おかげさまで大反響で、たくさんの方に読んでいただいたようで嬉しいです！
同様のドキュメントを一発生成＋自動更新するカスタムコマンド作りましたので、よければこちらでサクッと試してみてください。
...
.claude/00_project/01_appcadia_concept_requirements.md に配置
ハマりそうな外部ライブラリの使い方は Claude Opus や Deep Research を使って調査し、.claude/03_library_docs/ に配置
上記の内容を元に、Claude Code に各種ドキュメントを作らせる
